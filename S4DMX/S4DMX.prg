...
; S4DMX PROGRAM 
; =============	
; VERSION 20240718, FROM 20240626
;
; RUN INSIDE DMX-ETH MOTORS OF SPARC4
; Current version number stored on V33.
; See recent alterations at the end of this file.
;
; MEMORY MAP
; ----------
...
; V51 to V66: WPROT positions WP1-WP16. Written by ICS, used by SUBs 1-16.
...
; STATUS CODES: V46 < 10 
; ----------------------
; V46=0 READY, NONE SUBROUTINE IN EXECUTION
; V46=1 BUSY, SUBROUTINE IN EXECUTION
; V46=2 MOVING ON WPROT CYCLIC MODE.
;
; SPECIFIC ERRORS: V46 => 200
; ---------------------------
;	V46= 200/201			(PRG 0)	Invalid hardware ID
;	V46= 205					(SUB21)	ECHARPE mirror out of HOME.
;	V46= 210 TO 216		(PRG 1) WPROT automatic WP progress.
;
; ERROR CODES: 10 <= V46 <= 169 
; -----------------------------
; 3-DIGIT ERROR CODES ARE USED AS FOLLOWS:
; 1ST DIGIT:	=0 FOR INIT ERRORS, =1 FOR GOTO ERRORS, 
...
; V46: Motor status. <0 for INIT NOT DONE, =0 for READY, =1 for BUSY, or error code otherwise.
; V50: Mechanism hardware ID, set by S4DMX.
; --------------------------------------------
	V33 = 20240718		; Current S4DMX version.
	V46 = -2			 		; Set status to INIT not done condition
...
;
;=====================
SUB 0	; STATUS REQUEST - DEPRECATED
;=====================
;
; 20240718. The behavior of S4ICS became strange when running this SUB. 
; The status publication will be done by reading each variable one at a time.
;
; IDs, S4DMX VERSION, DMX-ETH VERSION, PX, EX, AND OTHERS VALUES CAN BE GET BY
; READING VARIABLES OR BY COMMANDS. THE OBJECTIVE OF THIS SUBROUTINE IS TO JOIN
; IN A SINGLE VARIABLE THE INFORMATION THAT WOULD TAKE MANY READINGS OTHERWISE.
; THE HARDWARE ID (V49) IS A KIND OF DATA THAT WOULD BE WELL PLACED ON V30, BUT
; IT WASN'T BECAUSE ITS LENGTH MAY GROW WITH ADDITION OF NEW MOTORS (ECHARPE).
; THE ECHARPE SENSOR IS CONNECTED TO THE DI2 OF GMIR.
; UNFORTUNATELY THE MOTOR ALARM CAN ONLY BE READ BY COMMAND AND NOT BY A PROGRAM.
;
; HOW TO GET THE S4CL STATUS FROM MOTOR
; MODE = ACTIVE IF THE COMMUNICATION WITH MOTOR IS OK, MODE=NONE OTHERWISE.
; CONDITION=NONE IF V30.init (V44) = 0. =READY IF V30.track=0. =BUSY IF V30.track=1. = TIMEOUT OTHERWISE. (V46)
; POSITION = EX (EXCEPT BY GFOC WHICH IS READ FROM V32)
; POSITION ID = V31 FROM WHICH ONE GETS THE POSITION NAME
; V31 = CURRENT PREDEFINED POSITION INDEX (-1, IF INVALID)
; V32 = GFOC EX
; V33 = CURRENT VERSION OF S4DMX
; V49 = HARDWARE ID
; V50 = SOFTWARE ID
;
; VARIABLES OF INTEREST -> 10ms to read 7 variables with Send and log instructions.
; V31, V32, V33, V44, V46, V49, V50
; REGISTERS OF INTEREST
; MST, DO, DI, EO, ALM, POL, SASTAT0, EX,PX, LTE, LTP, LTS 
;	Insert into V30 some information about motor and mechanism
;	Resulting bit fields are:
; NEW BITS: STATUS BITS 9-10, V49=V50, I'AM GFOC, SPARE 
;	struct	V30 {
;			int track:	 8		// original track and error codes of V46. = V46&255
;			int gfoc:		 1		// GFOC alive (=1 if V49&32=1)<<8
;			int idMatch: 1		// hardware id and software id match (=1 if V49=V50)<<9
;			int init:  	 2		// INIT done flags (from V44), =(GFOCinit OTHERSinit)<<10
;			int status:	11		// motor status, =(MST&2047)<<12
;			int spare:	 1		// spare bit
;			int io:      4		// motor inputs&outputs states, =(DO2 DO1 DI2 DI1)<<24
;			int latch:	 2		// latch status
;			int eo:      1		// motor driver state =(EO<<31)
;	100 calls to SUB 0 takes 25 seconds (DMX-ETH GUI running)
;--------------------
;
	V1 = V46					; Store tracking/error code
	IF V46 = 0
		V46 = 1			 		; Set start SUB code
	ENDIF
	V30 = 0						; Clear previous status
	V1 = V1 & 255			; Get its 1st 8 bits
	V30 = V30 + V1		; store V1
	V2 = V49 & 32			; id bit of GFOC
	IF V2 > 0
		V30 = V30 + 256	; set GFOC bit
	ENDIF
	IF V49=V50
		V30 = V30 + 512	; set idMatch bit
	ENDIF
	V3 = V44 & 31			; V44&31 is greater than zero if initialization was
	IF V3 > 0					; performed (except for GFOC). 
		V30 = V30 + 1024	; INIT done flag of others mechanisms at bit 14
	ENDIF
	V4 = V44 & 32			; Mask GFOC INIT done flag
	IF V4 > 0
		V30 = V30 + 2048	; GFOC INIT flag at bit 15
	ENDIF
	V11 = MSTX
	V5 = V11 & 2047		; Mask the 9 LSBs of motor status
	V5 = V5 << 12
	V30 = V30 + V5		; store V5
;
	V6 = DO
	...
	V30 = V30 + V6		; store V6
;
	V7 = LTSX					; Get latch status (2 bits)
	V7 = V7 & 3
	V7 = V7 << 28
	V30 = V30 + V7		; store V7
;
	IF EO = 1
		V30 = V30 + 1024	; 2^30 (bit 10)
	ENDIF
	V46 = V1					; Restore tracking/error code or clear start SUB code
ENDSUB
;
;
;===================================================
...
;
	...
	LSPD = 8000
	ACC = 100					; em 20240423, funcionando no OPD com ACC=100 
	DEC = 100
  EO = 1						; Enable motor driver
...
;	V71: maximum target position (= 1560)
; V75: Timeout counter, greater than 1560 (= 2000)
;	V76: overtravel pulses to eliminate backlash (= 10)
; V46: Motor status. <0 for INIT NOT DONE, =0 for READY, =1 for BUSY, or error code otherwise.
...
;
	...
	V76 = 10					; GFOC # overtravel pulses to eliminate backlash.
...
;
	...
	; Check for same hardware and software IDs and both not equal to zero
	; 20230918 - TEST:	REMOVED ALL SECURITY CHECKING, EXCEPT 
	; THE INIT DONE FLAG IN ORDER TO OPTIMIZE WPROT ROTATION.
	; BEFORE THIS MODIFICATION THIS ROUTINE WAST 0.3s TO START MOVING,
	; AND AFTER THIS MODIFICATION THAT TIME DROPPED TO ....
	;	IF V50 != V1
...
; V76: number of overtravel pulses to eliminate backlash4
; V4:  displacement
;
;
	V46 = 1			 			; Set start SUB code
...
; VERSION 20240718
; ================
; BOOT-UP (PRG 0) MAKES V46=-2 AND NOT =0. A UNIQUE AND SIMPLE MODIFICATION.
; WHEN NOT EXECUTING A MOVEMENT,S4ICS READS V46 AND ACTS AS FOLLOWS:
; IF V46 = -2 > CONDITION = NONE / INIT NOT DONE / MOTOR MAY HAVE BEEN TURNED OFF
; IF V46 = 0  > NORMAL CONDITION / S4ICS DOES NOT TAKE ANY ACTIONS
; IF V46 = 1  > CONDITION = TIMEOUT / IT COULDN'T BE BUSY IF S4ICS DIDN'T MOVE IT.
; OTHERWISE   > CONDITION = TIMEOUT / SOME ERROR HAS OCCURRED
;
; DETECTAR:
; VERSION 20240626
; ================
; BUG GFOC: OVERTRAVEL TO ELIMINATE BACKLASH WAS USING A WRONG VARIABLE (V74) INSTEAD OF V76
;           AS GMIR USES V74, THERE WAS AN INTERFERENCE BETWEEN THEN.
; GS0 RESPONSE (V30) HAS BEEN MODIFIED.
;
; VERSION 20240-00
; ================
; REMOVED V46 = 0	(Clear end SUB code) FROM THE END OF BOTH GS29 AND GS30
;
; VERSION 20240422
; ================
; New WPROT driver parameters:
;	ACC = 100 ; DEC = 100
;
; VERSION 20240123
; ================
; New WPROT driver parameters:
; HSPD = 160000	; LSPD = 8000 ; ACC = 200 ; DEC = 200 ;
;
; VERSION 20231215
; ================
; BUG GS0: NOW PREVIOUS ERROR (V46) IS SAVED AT BEGINNING OF GS0
;
; VERSION 20231201
; ================
; BUG GS0: 	V8 = V7 << 27	replaced by V8 = V7 << 25

; VERSION 20230904
; ================
; The start SUB flag (V46=1) added at begining of SUB1-16, and SUB29-30
; because S4ICS was reading  
;
; VERSION 20230822
; ================
; The values of the offsets between the reference sensor actuation point
; and the optical centering position of the mechanisms WPROT, WPSEL, and
; ASEL, respectively 900, -5000, and -200, was transfered from the 
; S4ICS configuration panel (pos_value[0]) to the INIT routines of these
; mechanisms (see V74). 
;
